...
public class StorageLocationController {
    ...
    @ApiResponses(
            value = {
                    ...
                    @ApiResponse(responseCode = "400", description = "Некорректные параметры или название места хранения уже существует."),
                    @ApiResponse(responseCode = "403", description = "Нет доступа."),
                    @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера.")
            }
    )
    ...
    // Обновление места хранения с обработкой ошибок
    @Operation(
            summary = "Обновление места хранения по ID",
            description = "Обновляет место хранения по ID. Доступно для ролей ADMIN и STOREKEEPER."
    )
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "200", description = "Место хранения успешно обновлено."),
                    @ApiResponse(responseCode = "400", description = "Некорректные параметры."),
                    @ApiResponse(responseCode = "403", description = "Нет доступа."),
                    @ApiResponse(responseCode = "404", description = "Место хранения не найдено."),
                    @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера.")
    ...
    public ResponseEntity<?> updateStorageLocation(
            @Parameter(description = "Идентификатор места хранения", required = true)
            @PathVariable String id,
            @Parameter(description = "Новое название места хранения (опционально)", required = false)
            @RequestParam(required = false) String name,
            @Parameter(description = "Новое описание места хранения (опционально)", required = false)
            @RequestParam(required = false) String description) {
        try {
            // Обновление места хранения через сервис
            StorageLocation updatedStorageLocation = storageLocationService.updateStorageLocation(id, name, description);
            ...
            // Обработка внутренней ошибки сервера
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Произошла ошибка при обновлении места хранения: " + e.getMessage());
        }
    }

    // Удаление места хранения с обработкой ошибок
    @Operation(
            summary = "Удаление места хранения по ID",
            description = "Удаляет место хранения по ID. Доступно для ролей ADMIN и STOREKEEPER."
    )
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "200", description = "Место хранения успешно удалено."),
                    @ApiResponse(responseCode = "403", description = "Нет доступа."),
                    @ApiResponse(responseCode = "404", description = "Место хранения не найдено."),
                    @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера.")
            }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteStorageLocation(
            @Parameter(description = "Идентификатор места хранения", required = true)
            @PathVariable String id) {
        try {
            // Удаление места хранения через сервис
            storageLocationService.deleteStorageLocation(id);
            return ResponseEntity.ok("Место хранения успешно удалено.");
        } catch (NoSuchElementException e) {
            // Обработка ошибки отсутствия места хранения
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        } catch (Exception e) {
            // Обработка внутренней ошибки сервера
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Произошла ошибка при удалении места хранения: " + e.getMessage());
        }
    }
}