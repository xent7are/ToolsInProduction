...
public class EmployeeService implements UserDetailsService {
    ...
    public Employee createEmployee(String positionId, String surname, String name, String patronymic,
        ...
        try {
            ...
            // Обработка нарушения уникальности email или номера телефона
            if (e.getMessage().contains("email")) {
                throw new IllegalArgumentException("Сотрудник с таким email уже существует.");
            } else if (e.getMessage().contains("phone_number")) {
                throw new IllegalArgumentException("Сотрудник с таким номером телефона уже существует.");
            }
            throw new IllegalArgumentException("Ошибка при сохранении сотрудника: нарушение уникальности данных.");
        }
    }
    ...
    public Employee updateEmployee(String idEmployee, String positionId, String surname, String name, String patronymic,
        ...
        if (phoneNumber != null && !phoneNumber.isEmpty()) {
            ...
            // Проверка уникальности номера телефона
            if (!phoneNumber.equals(employee.getPhoneNumber()) && employeeRepository.findByPhoneNumber(phoneNumber).isPresent()) {
                throw new IllegalArgumentException("Сотрудник с таким номером телефона уже существует.");
            }
        ...
        if (email != null && !email.isEmpty()) {
            ...
            // Проверка уникальности email
            if (!email.equals(employee.getEmail()) && employeeRepository.findByEmail(email).isPresent()) {
                throw new IllegalArgumentException("Сотрудник с таким email уже существует.");
            }
        ...
        if (login != null && !login.isEmpty()) {
            // Проверка уникальности логина
            if (!login.equals(employee.getLogin()) && employeeRepository.findByLogin(login).isPresent()) {
                throw new IllegalArgumentException("Сотрудник с таким логином уже существует.");
            }
        ...
        try {
            ...
            throw new IllegalArgumentException("Ошибка при обновлении сотрудника: нарушение уникальности данных.");
        }
    }
